...
<header class="barra-navegacao">
    ...
    @if(Auth::user()->foto)
        <a href="{{ route('perfil_index_estudante') }}"><img id="img-personalizada" src="{{ url('storage/' . Auth::user()->foto) }}" alt="imagem de perfil do usuario"></a>
    @else
        <a href="{{ route('perfil_index_estudante') }}"><img src="{{ asset('img/icones/perfil.png') }}" alt="imagem de perfil do usuario"></a>
    @endif
</header>
